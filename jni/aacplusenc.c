...
int aacplusEncSetConfiguration(aacplusEncHandle hEncoder,
	...
	LOGI("aacplusEncSetConfiguration 1 \n");
    ...
    if(!IsSbrSettingAvail(aacp->config.bitRate, aacp->config.nChannelsOut, sampleRateAAC, &sampleRateAAC)) {
		...
		sprintf(logstring, "libaacplus: bad aac setting: br:%d, AACch:%d, AACsr:%d\n",
                aacp->config.bitRate, aacp->config.nChannelsOut, sampleRateAAC);
		LOGI(logstring);
        return 0;
    }
	LOGI("aacplusEncSetConfiguration 2 \n");
	...
	LOGI("aacplusEncSetConfiguration 3 \n");
	...
	LOGI("aacplusEncSetConfiguration 4 \n");
//#ifdef _FFTW3
...
//#endif
	...
	LOGI("aacplusEncSetConfiguration 5 \n");
//#ifdef _FFTW3
...
int aacplusEncEncode(aacplusEncHandle hEncoder, int32_t *inputBuffer, unsigned int samplesInput,
    ...
    if (aacp->writtenSamples < aacp->config.inputSamples){
		...
		sprintf(logstr, "writtenSamples = %d, inputSamples=%d\n", aacp->writtenSamples, aacp->config.inputSamples);
		LOGI(logstr);
        return 0;
    }
